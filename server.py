    ...
    <script>
        ...
        function connectSocket() {
            ...
            socket.on('new_message', (data) => {
                playSound();
                if ((currentChatType==='group' && data.receiver===currentChat) || data.sender===currentChat || data.sender===username) addMessage(data);
            });
        ...
        function hideReactionPicker() { document.getElementById('reaction-picker').classList.remove('show'); document.removeEventListener('click', hideReactionPicker); }
        function addReaction(emoji) { if(reactionTarget){socket.emit('add_reaction',{message_id:reactionTarget,emoji:emoji,username:username});hideReactionPicker();} }
        ...
        function sendMessage() {
            ...
            if(editingMsgId){
                ...
                socket.emit('send_message',{sender:username,receiver:currentChatType==='group'?currentChat:currentChat,message:message,reply_to:replyingTo,is_group:currentChatType==='group'});
                cancelReply();
            }
            input.value='';
        }
        
        function editMsg(id, text) { editingMsgId=id;document.getElementById('message-input').value=text;document.getElementById('send-btn').textContent='Сохранить';document.getElementById('message-input').focus(); }
        ...
        function openCreateGroup() {
            ...
            fetch('/users').then(r=>r.json()).then(d=>{
                const list = document.getElementById('add-members-list'); list.innerHTML = '';
        ...
        function createGroup() {
            ...
            const members = []; document.querySelectorAll('.member-check:checked').forEach(cb => members.push(cb.value));
            if(members.length===0){alert('Выберите хотя бы одного участника');return;}
            fetch('/create_group',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,creator:username,members:members})})
            .then(r=>r.json()).then(d=>{if(d.status==='ok'){closeCreateGroup();socket.emit('group_created',{});}});
        }
        ...
        function openGroupInfo(groupId) {
            ...
            fetch(`/group_info/${groupId}`).then(r=>r.json()).then(d=>{
                ...
                d.members.forEach(m => {
                    ...
                    div.innerHTML = `<span>${m}</span>${d.creator===username&&m!==username?`<button class="btn-danger btn-small" style="width:auto;margin:0;" onclick="removeMember('${groupId}','${m}')">✕</button>`:''}`;
                    membersList.appendChild(div);
                });
                const addList = document.getElementById('add-member-list'); addList.innerHTML = '';
                fetch('/users').then(r=>r.json()).then(ud=>{
                    ud.users.forEach(u => {
                        if(u[0]!==username && !d.members.includes(u[0])){
                            const btn = document.createElement('button'); btn.className = 'btn-primary btn-small';
                            btn.style.cssText = 'width:auto;display:inline-block;margin:3px;';
                            btn.textContent = '+ '+u[0]; btn.onclick = () => addMember(groupId, u[0]);
        ...
        function addMember(groupId, member) {
            fetch('/add_member',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({group_id:groupId,username:member})})
            .then(()=>{openGroupInfo(groupId);socket.emit('group_updated',{});});
        }
        function removeMember(groupId, member) {
            fetch('/remove_member',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({group_id:groupId,username:member})})
            .then(()=>{openGroupInfo(groupId);socket.emit('group_updated',{});});
        }
        function leaveGroup() {
            if(!confirm('Покинуть группу?'))return;
            fetch('/leave_group',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({group_id:currentChat,username:username})})
            .then(()=>{closeGroupInfo();currentChat=null;document.getElementById('chat-header').textContent='👋 Добро пожаловать в Forevo';document.getElementById('messages').innerHTML='';socket.emit('group_updated',{});});
        }
        ...
        function logout(){
            if(socket)socket.disconnect();username=null;currentChat=null;currentChatType='user';
            document.getElementById('auth-area').style.display='block';document.getElementById('user-info').style.display='none';
            document.getElementById('chat-header').textContent='👋 Добро пожаловать в Forevo';
            document.getElementById('messages').innerHTML='';
            document.getElementById('message-input').disabled=true;document.getElementById('send-btn').disabled=true;
            document.getElementById('status').textContent='Не подключен';cancelReply();closeSettings();
        }
...
def set_avatar():
    data = request.json
    conn = get_db_connection(); c = conn.cursor()
    c.execute('UPDATE users SET avatar=%s WHERE username=%s', (data['avatar'], data['username']))
    conn.commit(); conn.close()
    return jsonify({'status': 'ok'})

@app.route('/delete_account', methods=['POST'])
def delete_account():
    data = request.json; username = data['username']
    conn = get_db_connection(); c = conn.cursor()
    c.execute('DELETE FROM messages WHERE sender=%s OR receiver=%s', (username, username))
    c.execute('DELETE FROM group_members WHERE username=%s', (username,))
    c.execute('DELETE FROM groups_chat WHERE creator=%s', (username,))
...
def create_group():
    data = request.json; name = data['name']; creator = data['creator']; members = data['members']
    conn = get_db_connection(); c = conn.cursor()
    c.execute('INSERT INTO groups_chat (name, creator) VALUES (%s, %s) RETURNING id', (name, creator))
    group_id = c.fetchone()[0]
    c.execute('INSERT INTO group_members (group_id, username) VALUES (%s, %s)', (group_id, creator))
    for m in members: c.execute('INSERT INTO group_members (group_id, username) VALUES (%s, %s) ON CONFLICT DO NOTHING', (group_id, m))
    conn.commit(); conn.close()
...
def my_groups(username):
    ...
    c.execute('''SELECT g.id, g.name, (SELECT COUNT(*) FROM group_members WHERE group_id=g.id) as cnt
                 FROM groups_chat g JOIN group_members gm ON g.id=gm.group_id WHERE gm.username=%s''', (username,))
    groups = [{'id': row[0], 'name': row[1], 'members_count': row[2]} for row in c.fetchall()]; conn.close()
    return jsonify({'groups': groups})

@app.route('/group_info/<int:group_id>', methods=['GET'])
def group_info(group_id):
    conn = get_db_connection(); c = conn.cursor()
    c.execute('SELECT name, creator FROM groups_chat WHERE id=%s', (group_id,))
    group = c.fetchone()
    c.execute('SELECT username FROM group_members WHERE group_id=%s', (group_id,))
    members = [row[0] for row in c.fetchall()]; conn.close()
...
def add_member():
    data = request.json
    conn = get_db_connection(); c = conn.cursor()
    c.execute('INSERT INTO group_members (group_id, username) VALUES (%s, %s) ON CONFLICT DO NOTHING', (data['group_id'], data['username']))
    conn.commit(); conn.close()
    return jsonify({'status': 'ok'})
...
def remove_member():
    data = request.json
    conn = get_db_connection(); c = conn.cursor()
    c.execute('DELETE FROM group_members WHERE group_id=%s AND username=%s', (data['group_id'], data['username']))
    conn.commit(); conn.close()
    return jsonify({'status': 'ok'})
...
def leave_group():
    ...
    conn = get_db_connection(); c = conn.cursor()
    c.execute('DELETE FROM group_members WHERE group_id=%s AND username=%s', (data['group_id'], data['username']))
    c.execute('SELECT creator FROM groups_chat WHERE id=%s', (data['group_id'],))
    row = c.fetchone()
    if row and row[0] == data['username']:
        c.execute('DELETE FROM group_members WHERE group_id=%s', (data['group_id'],))
        c.execute('DELETE FROM messages WHERE receiver=%s', (str(data['group_id']),))
        c.execute('DELETE FROM groups_chat WHERE id=%s', (data['group_id'],))
    conn.commit(); conn.close()
    return jsonify({'status': 'ok'})
...
def get_messages(user1, user2):
    conn = get_db_connection(); c = conn.cursor()
    c.execute('''SELECT id, sender, receiver, message, reply_to, reactions, edited, TO_CHAR(timestamp, 'HH24:MI') as ts
                 FROM messages WHERE deleted=0 AND receiver NOT LIKE 'g_%%' AND ((sender=%s AND receiver=%s) OR (sender=%s AND receiver=%s))
                 ORDER BY timestamp''', (user1, user2, user2, user1))
    msgs = [{'id':r[0],'sender':r[1],'receiver':r[2],'message':r[3],'reply_to':json.loads(r[4])if r[4]else None,'reactions':json.loads(r[5])if r[5]else{},'edited':r[6],'timestamp':r[7]} for r in c.fetchall()]; conn.close()
...
def get_group_messages(group_id):
    conn = get_db_connection(); c = conn.cursor()
    c.execute('''SELECT id, sender, receiver, message, reply_to, reactions, edited, TO_CHAR(timestamp, 'HH24:MI') as ts
                 FROM messages WHERE deleted=0 AND receiver=%s ORDER BY timestamp''', (str(group_id),))
    msgs = [{'id':r[0],'sender':r[1],'receiver':r[2],'message':r[3],'reply_to':json.loads(r[4])if r[4]else None,'reactions':json.loads(r[5])if r[5]else{},'edited':r[6],'timestamp':r[7]} for r in c.fetchall()]; conn.close()
    return jsonify({'messages': msgs})
...
def handle_login(data):
    ...
    join_room(username)
    conn = get_db_connection(); c = conn.cursor()
    c.execute('SELECT group_id FROM group_members WHERE username=%s', (username,))
    for row in c.fetchall(): join_room(str(row[0]))
    conn.close()
    emit('online_users', list(online_users.keys()), broadcast=True)
...
def handle_message(data):
    ...
    conn = get_db_connection(); c = conn.cursor()
    c.execute('INSERT INTO messages (sender, receiver, message, reply_to) VALUES (%s,%s,%s,%s) RETURNING id', (sender, receiver, message, reply_to))
    msg_id = c.fetchone()[0]; conn.commit(); conn.close()
    ...
    if not data.get('is_group'): emit('new_message', msg_data, room=receiver)

@socketio.on('delete_message')
def handle_delete(data):
    msg_id = data['id']
    conn = get_db_connection(); c = conn.cursor()
    c.execute('UPDATE messages SET deleted=1 WHERE id=%s', (msg_id,))
    c.execute('SELECT sender, receiver FROM messages WHERE id=%s', (msg_id,))
...
def handle_edit(data):
    ...
    conn = get_db_connection(); c = conn.cursor()
    c.execute('UPDATE messages SET message=%s, edited=1 WHERE id=%s', (new_message, msg_id))
    c.execute('SELECT sender, receiver FROM messages WHERE id=%s', (msg_id,))
    row = c.fetchone(); conn.commit(); conn.close()
    if row:
...
def handle_reaction(data):
    ...
    if row:
        ...
        c.execute('UPDATE messages SET reactions=%s WHERE id=%s', (json.dumps(reactions), msg_id))
        c.execute('SELECT sender, receiver FROM messages WHERE id=%s', (msg_id,))
        room_row = c.fetchone()
    conn.commit(); conn.close()
    if room_row:
...
def handle_group_created(data): emit('group_created', {}, broadcast=True)

@socketio.on('group_updated')
def handle_group_updated(data): emit('group_updated', {}, broadcast=True)

@socketio.on('avatar_changed')
def handle_avatar(data): emit('avatar_updated', data, broadcast=True)
